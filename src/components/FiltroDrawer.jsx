...
function FiltroDrawer({ filtros, setFiltros, onReset, visible, onClose, productosOriginales }) {
  ...
  useEffect(() => {
    ...
    const safeFiltros = {
      ...
      precio: {
        ...
        max: filtros.precio?.max >= 1000000 ? 50000 : (filtros.precio?.max ?? 50000),
      },
    };
  ...
  useEffect(() => {
    ...
    const algunEstadoSeleccionado = tempFiltros.estado.nuevo || tempFiltros.estado.usado || tempFiltros.estado.usadoComoNuevo;
    ...
    if (algunEstadoSeleccionado) {
      filtrados = filtrados.filter((p) => {
        ...
        if (tempFiltros.estado.usadoComoNuevo && p.estado === "Usado como nuevo") return true;
        return false;
      });
    }

    // Filtrar por precio
    const min = tempFiltros.precio.min !== "" && Number.isFinite(tempFiltros.precio.min) ? Number(tempFiltros.precio.min) : 0;
    const max = tempFiltros.precio.max !== "" && Number.isFinite(tempFiltros.precio.max) ? Number(tempFiltros.precio.max) : 1000000;
  ...
  const handlePrecioChange = (event, newValue) => {
    setTempFiltros((prev) => ({
      ...prev,
      // Si el slider está al máximo (50,000), usar 1,000,000 internamente para "sin límite"
      precio: { 
        min: newValue[0], 
  ...
  // Para el slider, si max es muy alto (1 millón), mostrar 50,000
  const precioValido = [min, max >= 1000000 ? 50000 : max];
    ...
    <>
      ...
      <nav className="flex-1 overflow-y-auto px-4 py-4 bg-gray-50 dark:bg-gray-800">
        ...
        <div className="mb-6">
          <Typography
            gutterBottom
            className="text-sm font-semibold text-blue-700 dark:text-blue-300 mb-3"
          >
            Precio (RD$)
          </Typography>
          <Box
          ...
          <div className="flex justify-between text-xs text-blue-700 dark:text-blue-300 mt-1 font-semibold">
            ...
            <span>{precioValido[1] >= 50000 ? 'RD$50,000+' : `RD$${precioValido[1].toLocaleString('es-DO')}`}</span>
          </div>
        </div>
