...
export default function BotonCardnet({ className, total, label, items }) {
  ...
  const formatted = Number.isFinite(displayAmount)
    ...
    : null;
  const buttonLabel =
    label || (formatted ? `Comprar ahora • ${formatted}` : "Comprar ahora");
  ...
  const iniciarPago = async () => {
    ...
    try {
      ...
      if (err.code === "deadline-exceeded") {
        errorMessage =
          "La pasarela de pago tardó mucho en responder. Por favor intenta de nuevo.";
      } else if (err.code === "failed-precondition") {
        errorMessage =
          "Hay un problema con la configuración de pagos. Contacta a soporte.";
      } else if (err.message) {
