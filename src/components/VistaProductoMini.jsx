...
const toMedia = (u) =>
  !u ? null : { type: isVideoUrl(u) ? "video" : "image", url: u };
...
const useMiniProduct = (productId, draftData) => {
  ...
  const merged = useMemo(() => {
    ...
    const extraMedia = Array.isArray(src.extraMedia)
      ? src.extraMedia
      : Array.isArray(src.imagenesExtra)
      ? src.imagenesExtra
      : [];
    ...
    const variants = variantsRaw.map((v, idx) => ({
      ...
      videos: Array.isArray(v.videos)
        ? v.videos
        : Array.isArray(v.videoUrls)
        ? v.videoUrls
        : v.video
        ? [v.video]
        : [],
...
const VistaProductoMini = ({ productId, draftData, className = "" }) => {
  ...
  const gallerySource =
    activeVariant &&
    (activeVariant.mainImage || (activeVariant.gallery || []).length)
      ? activeVariant
      : producto;

  const mainImage = gallerySource?.mainImage || "";
  const galleryList = Array.isArray(gallerySource?.gallery)
    ? gallerySource.gallery
    : [];
  // Añadir videos de la variante activa a la galería principal (si existen)
  const variantVideos = Array.isArray(activeVariant?.videos)
  ...
  const acercaList = Array.isArray(producto?.videoAcercaArticulo)
    ...
    : producto?.videoUrl
    ? [producto.videoUrl]
    : producto?.productVideos || [];
  const videoItems = acercaList
    ...
    <div className={`vista-producto-mini ${className}`}>
      ...
      <div className="mb-3">
        ...
        <p className="text-sm text-gray-500">
          Sin relacionados. Refleja cambios al instante.
        </p>
      </div>
        ...
        <div className="bg-white rounded-xl border shadow-sm p-4">
          ...
          <div className="flex items-baseline gap-2 mb-3">
            <span className="text-2xl font-semibold text-blue-700">
              RD$ {Number(price || 0).toLocaleString("es-DO")}
            </span>
            {typeof stock === "number" && (
              <span
          ...
          <div className="flex gap-2">
            <button
              type="button"
              className="flex-1 bg-blue-600 text-white rounded-lg py-2 text-sm shadow hover:bg-blue-700 transition-colors"
              disabled
            ...
            <button
              type="button"
              className="flex-1 bg-emerald-600 text-white rounded-lg py-2 text-sm shadow hover:bg-emerald-700 transition-colors"
              disabled
              onClick={(e) => e.stopPropagation()}
            >
              Comprar ahora
          ...
          <div className="flex flex-wrap gap-2">
            {producto.variants.map((v, i) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg border shadow-sm hover:shadow transition-all ${
                  i === varIndex
